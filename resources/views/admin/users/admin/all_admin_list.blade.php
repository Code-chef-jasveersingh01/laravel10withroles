...
@endcomponent
    <x-list_view>
        <x-slot name="card_style"> 0 </x-slot>
        <x-slot name="card_body_style"> 0 </x-slot>
        ...
        <x-slot name="table_id"> adminTable </x-slot>
        <x-slot name="table_th">
            ...
            <th>{{ __('main.role') }}</th>
            @canany(['View Admin Details', 'Edit Admin', 'Delete Admin'])
    ...
    $(document).ready(function () {
        $('#adminTable').DataTable({
            ...
            "columns"       : [
                                ...
                                {   "data": "roles",
                                    "render": function( data, type, row) {
                                        return (data.length > 0) ? data[0].name : '<span class="badge badge-soft-danger text-uppercase">{{__('main.no_role_found')}}</span>';
                                    }
                                },
                                @canany(['View Admin Details', 'Edit Admin', 'Delete Admin'])
                                {   "data": "uuid",
                                    "render": function ( data, type, row ) {
                                        return '@can("View Admin Details") <li class="list-inline-item edit"> <a href="{{route("admin.viewAdmin","")}}/'+row.uuid+'" data-id="'+row.uuid+'" class="text-primary d-inline-block edit-btn"> <i class="ri-eye-fill fs-16"></i> </a></li> @endcan @can("Edit Admin") <li class="list-inline-item edit"> <a href="{{route("admin.editAdmin","")}}/'+row.uuid+'" data-id="'+row.uuid+'" class="text-primary d-inline-block edit-btn"> <i class="ri-pencil-fill fs-16"></i></a></li> @endcan @can("Delete Admin") <li class="list-inline-item"> <a class="text-danger d-inline-block remove-item-btn"  data-id="'+row.uuid+'" href="javascript:void(0)"><i class="ri-delete-bin-5-fill fs-16"></i></a></li> @endcan';
                                    },
                                },
                                @endcanany
                            ],
