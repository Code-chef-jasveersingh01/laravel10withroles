...
class AdminController extends Controller
{
  ...
  {
      if (!auth_permission_check_any(['View All Admin', 'Create Admin','View Admin Details', 'Edit Admin', 'Delete Admin'])) abort(404);

      try {
          $adminTypes = User::all();
          return view('admin.users.admin.all_admin_list', compact('adminTypes'));
      } catch (\Exception $e) {
  ...
  {
      if (!in_array(Auth::user()->user_type, [1]) || !auth_permission_check('Create Admin')) abort(404);

      try {
          $adminTypes = UserType::where('level',1)->get();
          $roles = Role::all();
          return view('admin.users.admin.create_admin', compact('adminTypes','roles'));
      } catch (\Exception $e) {
  ...
  {
      if (!in_array(Auth::user()->user_type, [1]) || !auth_permission_check('View Admin Details')) abort(404);

      if (!User::where([['uuid', $uuid],['user_type','=',1]])->exists()) {
          Session::flash('alert-error', __('message.something_went_wrong'));
          return redirect()->back();
      }

      try {
          $adminDetails = User::with('media','userType','genderType')->where('uuid',$uuid)->first();
  ...
  {
      if (!in_array(Auth::user()->user_type, [1]) || !auth_permission_check('Edit Admin')) abort(404);
  ...
  {
      if (!in_array(Auth::user()->user_type, [1]) || !auth_permission_check('Delete Admin')) abort(404);

      DB::beginTransaction();
      try {
          User::where([['uuid','=',$uuid],['user_type','=',1]])->delete();
          DB::commit();
